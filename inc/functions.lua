...
----------------------{ Get Name Bot }-----------------------------
Bot_Name = redis:get(rambo..":NameBot:")
...
if redis:get(rambo..'group:add'..msg.chat_id_) then  return '🎗*¦* المجموعه بالتأكيد ✓️ تم تفعيلها' end
...
GetUserID(v.user_id_,function(arg,data)
redis:hset(rambo..'username:'..data.id_,'username', ResolveUserName(data))
redis:sadd(rambo..':MONSHA_BOT:'..arg.chat_id_,data.id_)
...
rambo..':lock_id_photo:'..arg.chat_id_,true
)
...
if cmd == "dn_sudo" then
if not redis:sismember(rambo..':SUDO_BOT:',UserID) then return sendMsg(ChatID,MsgID,'👤*¦* العضو » '..USERNAME..' \n🎫*¦* الايدي » (`'..UserID..'`)\n🛠*¦* انه بالتأكيد ليس مطور \n✓') end
...
if redis:get(rambo..'welcome:get'..msg.chat_id_) then welcomex = "{ مفعل }" else welcomex = "{ معطل }" end
if redis:get(rambo..'lock_id'..msg.chat_id_) then lock_idx = "{ مفعل }" else lock_idx = "{ معطل }" end
if redis:get(rambo..'replay'..msg.chat_id_)  then replayx  = "{ مفعل }" else replayx  = "{ معطل }" end
if redis:get(rambo..'lock_woring'..msg.chat_id_) then lock_woringx = "{ مفعل }" else lock_woringx  = "{ معطل }" end
...
.."\n🎙¦ البصمات » "..(redis:get(rambo..'mute_audio'..msg.chat_id_) or 'false')
.."\n\n🔉¦ الصوت » "..(redis:get(rambo..'mute_voice'..msg.chat_id_) or 'false')
